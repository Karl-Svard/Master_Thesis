...
def set_up_demography(t12, t120, tBT, N_bn, N):
    ...
    divergence = [msp.MassMigration(time=t12,source=P2,destination=P1,proportion=1),
                  ...
                  msp.PopulationParametersChange(time=t12, initial_size=anc_N, growth_rate=growth_rate, population=P1),
                  msp.PopulationParametersChange(time=t12+exp_years/generation_time, initial_size=anc_N0, growth_rate=None, population=P1)]
    # admixture times and proportion
    #admix = [msp.MassMigration(time=ta,source=P1,destination=P2,proportion=f)]
    admix2 = [msp.MigrationRateChange(time=0.95*t12, rate=0.6, matrix_index=(P1,P2)),
...
# Parameters for ancestral expansion event
anc_N = 10000
growth_rate = 1.1e-4
exp_years = 10000
anc_N0 = anc_N*math.exp(growth_rate*exp_years)
...
np.save('output/norm_10k_anc_shr.npy',norm_arr)
np.save('output/bound_10k_anc_shr.npy',bound_arr)
np.save('output/conv_10k_anc_shr.npy',conv_arr)
np.save('output/TTo_10k_anc_shr.npy',TTo_arr)
np.save('output/TT_10k_anc_shr.npy',TT_arr)
