...
def set_up_demography(t12, t120, tBT, N_bn, N):
    ...
    divergence = [msp.MassMigration(time=t12,source=P2,destination=P1,proportion=1),
                  ...
                  msp.PopulationParametersChange(time=tBT[0], initial_size=N_bn, growth_rate=None, population=P1),
                  msp.PopulationParametersChange(time=tBT[1], initial_size=N, growth_rate=None, population=P1)]
...
tBT = [1.5*t12, 1.8*t12] # start and end date of bottleneck
...
np.save('output/norm_10k_anc_bottle.npy',norm_arr)
np.save('output/bound_10k_anc_bottle.npy',bound_arr)
np.save('output/conv_10k_anc_bottle.npy',conv_arr)
np.save('output/TTo_10k_anc_bottle.npy',TTo_arr)
np.save('output/TT_10k_anc_bottle.npy',TT_arr)
