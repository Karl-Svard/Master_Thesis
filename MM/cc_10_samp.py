...
nS=[10,10] # number of samples per population
...
configs = eq.get_sample_sets(nS[0],nS[1])
...
for i, sim in enumerate(sims):
    M = config_counts(configs, sim, [nS[0],nS[1]])
    print(M)
    nA=nS[0]
    nB=nS[1]
...
np.save('output/mm_results_0admx_1010.npy',res_list)
np.save('output/bval_0admx_1010.npy',bval_list)
